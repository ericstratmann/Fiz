...
/**
 * A TableLayout can be used to lay out HTML fragments on a page. Layouts are
 * specified in the form of a grid of ASCII characters, which is expanded
 * internally into HTML.
 * For example, the following layout specifies how to display HTML elements
 ...
 * +------+------+
 * </pre>
 * The HTML for the child elements is generated by invoking a callback on the
 * {@link org.fiz.TableLayoutContainer} object that is passed to the
 * {@code TableLayout.html} method. 
 */
public class TableLayout {
    ...
    protected static class HtmlFragment {
        enum FragmentType {
            ...
            TemplateHtml,
            // Expand by invoking the childHtml method on the container object.
            ChildId,
         ...
         * {@code type}, this is either a template, the id of a child HTML
         * element that must be expanded, or plain HTML.
         */
        String value;
    ...
    // No constructor: this class has only static methods. 
    private TableLayout() {}
    ...
    /**
     * Generates HTML for a table layout specified using the {@code properties}
     * dataset.
     * The following properties are supported:
     *   id:             (optional)  Used as the {@code id} attribute for the
     *                   HTML table. Used to find the table in Javascript, e.g.
     *                   to make it visible or invisible.  Must be unique among
     *                   all id's for the page.
     *   class:          (optional) Used as the {@code class} attribute for
     *                   the HTML table.
     *   layout:         (required) A layout description that will be used to
     *                   generate the HTML table.
     * 
     * The {@code childHtml} method of {@code container} is used to
     * generate HTML for a child element. The generated HTML is appended to the
     * Html object associated with {@code cr}.
     *
     * @param properties        Contains configuration properties for
     *                          expanding a table layout. 
     * @param container         The {@link org.fiz.TableLayoutContainer} object
     *                          whose {@code childHtml} method is invoked to
     ...
     *                          layout specified in {@code properties}.
     */
    public static void html(Dataset properties,
        ...
        synchronized (layoutCache) {
            parsedLayout = layoutCache.get(layout);
            if (parsedLayout == null) {
                // Create a new instance of a TableLayout object
                // and cache it for later use.
                parsedLayout = parse(layout);
                layoutCache.put(layout, parsedLayout);
            } else {
        ...
        for (HtmlFragment fragment : parsedLayout) {
            switch (fragment.type) {
                case TemplateHtml:
                    ...
                    break;
                case ChildId:
                    container.childHtml(fragment.value, cr);
                    break;
                case PlainHtml:
        ...
        for (int r = 0; r < grid.length - 2; r++) {
            ...
            for (int c = 0; c < grid[r].length - 2; c++) {
                ...
                parsedLayout.add(new HtmlFragment(cellName,
                        HtmlFragment.FragmentType.ChildId));
