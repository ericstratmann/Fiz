...
public class DemoInteractor extends Interactor {
    ...
    public void stats(ClientRequest cr) {
        Html html = cr.getHtml();
        html.setTitle("org.fiz.Timer Statistics");
        ArrayList<Dataset> stats = org.fiz.Timer.getStatistics(1.0e06, "%.3f");
        ...
        cr.showSections(
                new TemplateSection("<h1>org.fiz.Timer Statistics</h1>\n"),
                new TableSection(
                        new Dataset("data", statsDataset),
                        new Column("org.fiz.Timer", "@name"),
    ...
    public void ajaxClearStats(ClientRequest cr) {
        org.fiz.Timer.resetAll();
        org.fiz.Timer.measureNoopTime();
        // Only get garbage collection info if not running under Google
        // AppEngine - ManagementFactory is not on the AppEngine whitelist.
        if (!cr.isGoogleAppEngine()) {
            long gcMs = 0;
