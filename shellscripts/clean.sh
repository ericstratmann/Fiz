...
# The program is run like: bash clean.sh [check|fix] path/to/file
# If the first argument is "check", the script simply finds the occurance of
# these errors and reports on them. If passed in the string "fix", it attempts
# to fix the errors when possible (though not all errors can be fixed).
